...
class FrontierAgent(Agent):
    ...
    def __init__(self, collection):
        ...
        # Lazy model initialization (fixes meta tensor error on macOS)
        self.model = None
    ...
    def find_similars(self, description: str):
        ...
        # Lazy-load the SentenceTransformer model (fixes NotImplementedError)
        if self.model is None:
            os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"
            self.log("Loading SentenceTransformer model lazily (CPU mode for safety)")
            self.model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2', device='cpu')

        # Encode the input description and query Chroma
